...
function getLast3MonthDateRange(monthsAgo) {
    const now = new Date();
    const fromDate = new Date(now.getFullYear(), now.getMonth() - monthsAgo, 1);
    //const toDate = monthsAgo === 0
    //    ? now
...
async function initializePowerFactorData() {
    ...
    try {
        ...
        let mainCirclesData = [];
        let highLowPFData = [];
        const dateRange = getLast3MonthDateRange(3);
...
// ----------- Average Power Factor Bar Chart -----------

let barChartData = [];
let selectedDeviceNames = new Set();
let deviceColorMap = new Map();

function processBarChartData(data, monthLabel) {
    if (!data || data.length === 0) return [];

    return data.map((row, index) => ({
        month: monthLabel,
        weekIndex: row.WeekNo || (index + 1),
...
function createBarChartOption(filteredData) {
    ...
    filteredData.forEach((monthData, monthIndex) => {
        ...
        monthData.rows.forEach((row) => {
            ...
            seriesData.push({
                ...
                rowData: row // Store only this bar's data
            });
        ...
        if (monthIndex < filteredData.length - 1) {
    ...
    return {
        tooltip: {
            trigger: 'item', // Changed from 'axis' to 'item' to show only single bar
            formatter: function (params) {
                ...
                // Build HTML for just this SINGLE bar
                let html = `
            <div style="background: rgba(0,0,0,0.9); border-radius: 8px; padding: 12px; min-width: 200px;">
                <div style="color: #66BB6A; font-weight: bold; margin-bottom: 12px; font-size: 14px; text-align: center; border-bottom: 2px solid #66BB6A; padding-bottom: 8px;">
                    ${row.month} - ${row.deviceName}
                </div>
                <div style="margin-bottom: 0; padding: 6px; background: rgba(255,255,255,0.05); border-radius: 4px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        <span style="color: #FFD54F;"><strong>Week:</strong></span>
                        <span style="color: #fff; font-weight: bold;">${row.weekIndex}</span>
                    </div>
            ...
            axisLabel: {
                ...
                formatter: function (value, index) {
                    for (const boundary of monthBoundaries) {
                        ...
                        if (index === centerIndex) return boundary.month;
                    }
                    return '';
                }
            },
...
function updateBarChart() {
    const filteredData = barChartData.map(monthData => ({
        month: monthData.month,
        rows: monthData.rows.filter(row => selectedDeviceNames.has(row.deviceName))
...
async function initializeBarChart() {
    ...
    const monthInfo = [
        { range: getMonthDateRange(3), label: getMonthName(3), index: 0 },
        { range: getMonthDateRange(2), label: getMonthName(2), index: 1 },
        { range: getMonthDateRange(1), label: getMonthName(1), index: 2 },
        { range: getMonthDateRange(0), label: getMonthName(0), index: 3 }
    ];

    const allMonthlyData = new Array(4);
    const allDeviceNames = new Set();

    try {
        const promises = monthInfo.map(async (info) => {
            const endpoint = `/EnergyParameter/GetAllmeter_WeeklyAvgPowerfactor_cold?fromDate=${encodeURIComponent(info.range.from)}&toDate=${encodeURIComponent(info.range.to)}`;

            try {
                const data = await fetchWithRetry(endpoint);
                const monthRows = processBarChartData(data, info.label);
                monthRows.forEach(row => allDeviceNames.add(row.deviceName));
                allMonthlyData[info.index] = { month: info.label, rows: monthRows };
            } catch (error) {
                console.error(`Error fetching ${info.label} data:`, error);
                allMonthlyData[info.index] = { month: info.label, rows: [] };
            }
        });

        await Promise.all(promises);

        barChartData = allMonthlyData.filter(m => m && m.rows.length > 0);

        const deviceArray = Array.from(allDeviceNames);
        deviceArray.forEach((deviceName, index) => {
            deviceColorMap.set(deviceName, getDeviceColor(deviceName, index));
        });
