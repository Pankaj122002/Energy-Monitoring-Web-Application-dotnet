...
// High / Low current sections: Avgcurrent API → show Count, NO Time
...
function normaliseRow(row) {
    // GetAllmeter_Avgcurrent: Devicename, DeviceID, Count, TotalCurrent, R_current, Y_current, B_current, Date
...
function rendercurrentDistributionPie(allDevices) {
    ...
    chart.setOption({
        ...
        tooltip: {
            ...
            formatter: p => `<div style="padding:4px 2px;"><div style="color:#A9D4FB;font-weight:bold;font-size:13px;margin-bottom:6px;">${p.name}</div><div style="color:#ccc;">Device ID: <strong style="color:#fff;">${devMap[p.name] || '-'}</strong></div><div style="color:#ccc;margin-top:3px;">TotalCurrent: <strong style="color:#81C784;">${p.value.toFixed(2)} A</strong></div><div style="color:#ccc;margin-top:3px;">Share: <strong style="color:#A9D4FB;">${p.percent.toFixed(2)}%</strong></div></div>`
        },
        legend: {
...
// High & Low sections use GetAllmeter_Avgcurrent — sorted by current desc
(async function initHighLowcurrent() {
    ['high-current-grid', 'low-current-grid'].forEach(id => showState(id, 'loading', '⏳ Loading...'));
    try {
        const data = await fetchWithRetry('/EnergyParameter/GetAllmeter_AvgCurrent?timeCategory=hot');
        if (!data.length) {
...
(async function initOpsSchedule() {
    ...
    try {
        const data = await fetchWithRetry('/EnergyParameter/GetAllMeters_AvgCurrent_Operationwise?timeCategory=hot');
...
// ============================================================
let barcurrentData = [], hourlycurrentData = [], todaycurrentData = [];
...
function processBarData(data) {
    return data.map(row => ({
        ...
        // Bars driven by current column only
        avgcurrent: row.TotalCurrent !== undefined ? parseFloat(row.TotalCurrent).toFixed(2) : '0.00',
...
function updateBarChart() {
    ...
    const series = Object.keys(groups).map(name => ({
        ...
        data: hours.map(h => { const fd = groups[name].find(r => r.hourDisplay === h); return fd ? { value: parseFloat(fd.avgcurrent), rowData: fd } : null; })
    }));
    const c = echarts.getInstanceByDom(el) || echarts.init(el);
    c.setOption({
        tooltip: { trigger: 'item', formatter(p) { if (!p.data?.rowData) return ''; const r = p.data.rowData; return `<div style="background:rgba(0,0,0,.9);border-radius:8px;padding:12px;min-width:210px;"><div style="color:#66BB6A;font-weight:bold;margin-bottom:8px;">Hour ${r.hourDisplay} – ${r.deviceName}</div><div style="color:#A9D4FB"><strong>Device ID:</strong> <span style="color:#fff">${r.deviceId}</span></div><div style="color:#A9D4FB;margin-top:4px"><strong>Date:</strong> <span style="color:#fff">${r.date}</span></div><div style="color:#A9D4FB;margin-top:4px"><strong>Current:</strong> <span style="color:#81C784;font-size:15px;font-weight:bold">${r.avgcurrent} A</span></div><div style="border-top:1px solid #444;margin-top:6px;padding-top:6px;"><div style="color:#EF9A9A;margin-top:2px"><strong>R Phase:</strong> <span style="color:#fff">${r.r_current} A</span></div><div style="color:#FFF176;margin-top:2px"><strong>Y Phase:</strong> <span style="color:#fff">${r.y_current} A</span></div><div style="color:#90CAF9;margin-top:2px"><strong>B Phase:</strong> <span style="color:#fff">${r.b_current} A</span></div></div></div>`; }, backgroundColor: 'transparent', borderColor: 'transparent', confine: true },
...
async function initBarChart() {
    ...
    try {
        const data = await fetchWithRetry(`/EnergyParameter/GetAllmeter_AvgCurrent_bargraph?timeCategory=hot`);
        ...
        DeviceColors.registerAll(names); buildCheckboxes('current-bar-filter-checkboxes', names, selBarDevs, updateBarChart); updateBarChart();
...
function renderHourFilter() {
    const container = document.getElementById('current-hourly-hour-filter'); if (!container) return;
...
function buildLineSeries(sorted, timeMap) {
    ...
    return Object.keys(groups).map(name => {
        ...
        const data = timeMap.map(tp => {
            ...
            return {
                value: parseFloat(m.TotalCurrent || 0),
                deviceId: m.DeviceID || '-',
                r_current: m.R_Current !== undefined ? parseFloat(m.R_Current).toFixed(2) : '-',
                y_current: m.Y_Current !== undefined ? parseFloat(m.Y_Current).toFixed(2) : '-',
                b_current: m.B_Current !== undefined ? parseFloat(m.B_Current).toFixed(2) : '-',
            };
        });
...
function lineTooltip() {
    return {
        ...
        formatter(params) {
            ...
            h += `<div style="color:#A9D4FB;font-weight:bold;margin-bottom:10px;text-align:center;font-size:13px;">${params[0].name}</div>`;
            params.forEach(p => {
...
function parseActualcurrentTime(row) {
    // GetAllmeter_Actualcurrent returns separate Date and Time columns
...
function updateHourlyChart() {
    const el = document.getElementById('chart-hourly-current'); if (!el) return;
    if (!selHourlyDevs.size) { chartState(el, 'nodevice', 'Please select a device'); return; }
    let f = hourlycurrentData.filter(r => selHourlyDevs.has(r.Devicename || r.DeviceName || 'Unknown'));
    if (selHour !== null && selHour !== undefined && selHour !== '') {
        f = f.filter(r => parseActualcurrentTime(r).getHours() === selHour);
    }
    ...
    const c = echarts.getInstanceByDom(el) || echarts.init(el);
    c.setOption({ tooltip: lineTooltip(), grid: { left: 60, right: 40, top: 40, bottom: 20 }, xAxis: { type: 'category', data: timeMap.map(t => t.str), axisLabel: { show: false }, boundaryGap: false }, yAxis: { type: 'value', name: 'A', axisLabel: { fontSize: 12, color: '#666' }, splitLine: { lineStyle: { type: 'dashed', color: '#eee' } } }, series: buildLineSeries(sorted, timeMap), animation: false }, true);
}
...
async function initHourlyChart() {
    ...
    try {
        ...
        const data = await fetchWithRetry(`/EnergyParameter/GetAllmeter_ActualCurrent?timeCategory=hot`);
        if (!data.length) { chartState(el, 'nodata', 'No data available'); return; }
        hourlycurrentData = data;
        ...
        buildCheckboxes('current-hourly-filter-checkboxes', names, selHourlyDevs, updateHourlyChart);
...
function updateTodayChart() {
    ...
    const f = todaycurrentData.filter(r => selTodayDevs.has(r.Devicename || r.DeviceName || 'Unknown'));
    if (!f.length) { chartState(el, 'nodevicedata', 'No data for the selected devices'); return; }
    const sorted = f.slice().sort((a, b) => parseActualcurrentTime(a) - parseActualcurrentTime(b));
    // Build a deduped union of all timestamps across all selected devices
    const tsSet = new Map();
...
async function initTodayChart() {
    ...
    try {
        ...
        buildCheckboxes('current-today-filter-checkboxes', names, selTodayDevs, updateTodayChart);
        updateTodayChart();
    } catch (e) { chartState(el, 'error', '⚠️ Error fetching data'); }
    window.addEventListener('resize', () => echarts.getInstanceByDom(el)?.resize());
}
