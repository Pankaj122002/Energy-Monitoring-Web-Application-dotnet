// ============================================================
//  powerWarm.js  –  Warm category power dashboard
//  Only the two chart sections are warm-specific:
//    1. Average Power – Daily bar chart  (week-wise x-axis)
//    2. Actual Power  – Full-month line chart with date filter
//  All other sections (circles, pie, high/low, ops) reuse
//  the same helper functions from power.js approach but call
...
function _buildCircleElement(device, tooltipRows) {
    ...
    DeviceColors.get(device.name); FFE082
    ...
    const series = Object.keys(groups).filter(name => selBarDevs.has(name)).map(name => {
        ...
        return {
            ...
            data: xLabels.map(lbl => {
                ...
                const rd = g[lbl];
                return rd ? { value: parseFloat(rd.avgPower), rowData: rd } : null;
            })
        };
    });
        ...
        tooltip: {
            ...
            formatter(p) {
                ...
                return `<div style="background:rgba(0,0,0,.9);border-radius:8px;padding:12px;min-width:210px;">
                    ...
                    <div style="color:#FFE082"><strong>Device ID:</strong> <span style="color:#fff">${r.deviceId}</span></div>
                    <div style="color:#FFE082;margin-top:4px"><strong>Date:</strong> <span style="color:#fff">${r.fullDate}</span></div>
                    <div style="color:#FFE082;margin-top:4px"><strong>Power:</strong> <span style="color:#81C784;font-size:15px;font-weight:bold">${r.avgPower} W</span></div>
                    <div style="border-top:1px solid #444;margin-top:6px;padding-top:6px;">
                        <div style="color:#EF9A9A;margin-top:2px"><strong>R Phase:</strong> <span style="color:#fff">${r.r_power} W</span></div>
                        <div style="color:#FFF176;margin-top:2px"><strong>Y Phase:</strong> <span style="color:#fff">${r.y_power} W</span></div>
                        <div style="color:#90CAF9;margin-top:2px"><strong>B Phase:</strong> <span style="color:#fff">${r.b_power} W</span></div>
                    </div></div>`;
            },
...
async function initBarChart() {
    ...
    try {
        const data = await fetchWithRetry(`/EnergyParameter/GetAllmeter_AvgPower_bargraph?timeCategory=warm`);
        ...
        buildCheckboxes('power-bar-filter-checkboxes', names, selBarDevs, updateBarChart);
        updateBarChart();
...
// ============================================================
//  2. ACTUAL POWER – FULL MONTH LINE CHART  (date filter)
// ============================================================
let actualPowerData = [];
...
function lineTooltipWarm() {
    return {
        ...
        formatter(params) {
            ...
            h += `<div style="color:#FFE082;font-weight:bold;margin-bottom:10px;text-align:center;font-size:13px;">${timeStr}</div>`;
            params.forEach(p => {
                const v = p.value[1];
                if (v == null) return;
    ...
    return Object.keys(groups).map(name => {
        const data = groups[name].map(d => {
            ...
            return {
                ...
                value: [
                    t,
                    parseFloat(d.Power || 0)
                ],
                deviceId: d.DeviceID || '-',
                r_power: d.R_Power !== undefined ? parseFloat(d.R_Power).toFixed(2) : '-',
                y_power: d.Y_Power !== undefined ? parseFloat(d.Y_Power).toFixed(2) : '-',
                b_power: d.B_Power !== undefined ? parseFloat(d.B_Power).toFixed(2) : '-'
            };
        });
...
function updateActualChart() {
    ...
    let f = actualPowerData.filter(r => selTodayDevs.has(r.Devicename || r.DeviceName || 'Unknown'));
    if (warmSelectedDate) {
...
async function initActualChart() {
    ...
    echarts.init(el); chartState(el, 'loading', '⏳ Loading...');
    try {
        const data = await fetchWithRetry(`/EnergyParameter/GetAllmeter_ActualPower?timeCategory=warm`);
        if (!data.length) { chartState(el, 'nodata', 'No data available'); return; }
        actualPowerData = data;
        ...
        DeviceColors.registerAll(names);
        buildCheckboxes('power-today-filter-checkboxes', names, selTodayDevs, updateActualChart);
        updateActualChart();
    } catch (e) { chartState(el, 'error', '⚠️ Error fetching data'); }
    window.addEventListener('resize', () => echarts.getInstanceByDom(el)?.resize());
}
...
(async function initPowerRealtime() {
    showState('power-grid-1', 'loading', '⏳ Loading...');
    ...
    try {
        const data1 = await fetchWithRetry('/EnergyParameter/GetAllMeters_LastUpdatedPower_piegraph');
        if (!data1.length) {
        ...
        if (!data.length) {
            showState('power-grid-1', 'nodata', 'No data available');
            return;
        }
        ...
        showState('power-grid-1', 'error', '⚠️ Error fetching data');
...
(async function initHighLowPower() {
    ...
    try {
        const data = await fetchWithRetry('/EnergyParameter/GetAllmeter_AvgPower?timeCategory=warm');
        if (!data.length) { ['high-power-grid', 'low-power-grid'].forEach(id => showState(id, 'nodata', 'No data available')); return; }
        const all = data.map(normaliseRow);
        DeviceColors.registerAll(all.map(d => d.name));
    ...
    } catch (err) { ['high-power-grid', 'low-power-grid'].forEach(id => showState(id, 'error', '⚠️ Error fetching data')); }
})();

(async function initOpsSchedule() {
    showState('ops-schedule-grid', 'loading', '⏳ Loading...');
    try {
        const data = await fetchWithRetry('/EnergyParameter/GetAllMeters_AvgPower_Operationwise?timeCategory=warm');
