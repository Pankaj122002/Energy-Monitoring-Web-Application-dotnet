...
function getLastMonthRange() {
    ...
    return {
        from3: formatDate(fromDate),
        to3: formatDate(toDate)
    };
}
...
function getMonthDateRange(monthsAgo) {
    const now = new Date();
...
function processAllRows(data, monthLabel) {
...
function createGroupedBarChartOption(allMonthlyData) {
    ...
    allMonthlyData.forEach((monthData, monthIndex) => {
        // Mark the start of each month group
        monthBoundaries.push({
            start: currentIndex,
            end: currentIndex + monthData.rows.length - 1,
        ...
        // Add spacing between month groups (except after last month)
        if (monthIndex < allMonthlyData.length - 1) {
            seriesData.push({
                value: null,
                itemStyle: { opacity: 0 }
            });
        ...
        xAxis: {
            ...
            axisLabel: {
                ...
                formatter: function (value, index) {
                    // Only show label at the center of each month group
...
// Initialize combined 4-month bar chart with all rows
document.addEventListener('DOMContentLoaded', function () {
    const chartAvgVoltage = echarts.init(document.getElementById('chart-avg-voltage'));

    // Get 4 month date ranges
    const month3Range = getMonthDateRange(3);
    const month2Range = getMonthDateRange(2);
    const month1Range = getMonthDateRange(1);
    const month0Range = getMonthDateRange(0);

    const allMonthlyData = [];
    let completedRequests = 0;

    const monthInfo = [
        { range: month3Range, label: getMonthName(3), apiEndpoint: 'GetMainmeter_WeeklyAvgVoltage_cold', index: 0 },
        { range: month2Range, label: getMonthName(2), apiEndpoint: 'GetMainmeter_WeeklyAvgVoltage_cold', index: 1 },
        { range: month1Range, label: getMonthName(1), apiEndpoint: 'GetMainmeter_WeeklyAvgVoltage_cold', index: 2 },
        { range: month0Range, label: getMonthName(0), apiEndpoint: 'GetMainmeter_WeeklyAvgVoltage_cold', index: 3 }
    ];

    monthInfo.forEach((info) => {
        const endpoint = `/EnergyParameter/${info.apiEndpoint}?fromDate=${encodeURIComponent(info.range.from)}&toDate=${encodeURIComponent(info.range.to)}`;

        fetch(endpoint)
            .then(response => response.json())
            .then(data => {
                console.log(`${info.label} raw data:`, data);
                ...
                if (typeof data === 'string') {
                    try {
                        ...
                        console.error(`Failed to parse ${info.label} data:`, e);
                        parsedData = [];
                    }
                }

                // Process all rows from this month
                const monthRows = processAllRows(parsedData, info.label);

                allMonthlyData[info.index] = {
                    month: info.label,
                    rows: monthRows
                };

                console.log(`${info.label} processed ${monthRows.length} rows:`, allMonthlyData[info.index]);

                completedRequests++;

                // Once all 4 APIs have responded, render chart
                if (completedRequests === 4) {
                    const validData = allMonthlyData.filter(m => m && m.rows.length > 0);

                    if (validData.length > 0) {
                        console.log('Combined monthly data for chart:', validData);
            ...
            .catch(error => {
                console.error(`Error fetching ${info.label} data:`, error);
                completedRequests++;
...
//    fetch(`/EnergyParameter/GetMainMeter_ActualVoltage?fromDate=${encodeURIComponent(from)}&toDate=${encodeURIComponent(to)}`)
//        .then(response => response.json())
//        .then(data => {
