...
var hvacThermalDashboard = (function () {
    ...
    function ensureLiveCache(assets, callback) {
        ...
        assets.forEach(function (asset) {
            ...
            $.ajax({
                ...
                url: '/Thermal/GetThermalMonitoringHVAC_live_ByAssetId?assetid=' + encodeURIComponent(id),
                success: function (result) {
                    var response = typeof result === 'string' ? JSON.parse(result) : result;
    ...
    function ensureAvgCache(assets, callback) {
        ...
        assets.forEach(function (asset) {
            ...
            $.ajax({
                type: 'POST',
                // C# reads assetid from query string: ?assetid=
                url: '/Thermal/GetThermalMonitoringHVAC_AverageTrends_Hot?assetid=' + encodeURIComponent(id),
                success: function (result) {
                    var response = typeof result === 'string' ? JSON.parse(result) : result;
    ...
    function g2_render(rows, selectedIds) {
        ...
        selectedIds.forEach(function (assetId) {
            ...
            var rgbColor = hexToRgbInline(color);
        ...
        g2.chartInstance.setOption({
            tooltip: {
                trigger: 'axis',
                backgroundColor: 'rgba(40,10,20,0.88)', borderColor: 'rgba(224,92,138,0.4)', borderWidth: 1,
                textStyle: { color: '#fff', fontSize: 12 },
                formatter: function (params) {
                    if (!params.length) return '';
                    var html = '<div style="font-weight:bold;margin-bottom:6px;border-bottom:1px solid rgba(224,92,138,0.4);padding-bottom:4px;">Hour: ' + params[0].name + ':00</div>';
                    params.forEach(function (p) {
                        html += '<div style="margin:3px 0;"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:' + p.color + ';margin-right:6px;"></span>' +
            ...
            xAxis: { type: 'category', data: xLabels, name: 'Hour', nameLocation: 'middle', nameGap: 30, axisLabel: { fontSize: 9, color: '#666' }, axisLine: { lineStyle: { color: '#e8a0b8' } }, splitLine: { show: true, lineStyle: { color: 'rgba(255,182,210,0.18)', type: 'dashed' } } },
            yAxis: { type: 'value', name: 'Temperature (°C)', nameLocation: 'middle', nameGap: 50, nameRotate: 90, nameTextStyle: { fontSize: 12, fontWeight: 'bold', color: '#333' }, axisLabel: { formatter: '{value}°', fontSize: 10, color: '#666' }, splitLine: { show: true, lineStyle: { color: 'rgba(255,182,210,0.22)', type: 'dashed' } } },
            dataZoom: [{ type: 'inside', start: 0, end: 100 }, { type: 'slider', show: true, start: 0, end: 100, height: 15, bottom: 10, fillerColor: 'rgba(224,92,138,0.12)', borderColor: 'rgba(224,92,138,0.3)' }],
            series: series,
            animation: true
        }, true);
    }
