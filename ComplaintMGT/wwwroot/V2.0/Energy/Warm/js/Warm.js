...
function checkCurrentSlideForGraph3() {
    var activeSlide = document.querySelector('#carouselEnergyTrendHotIndicators .carousel-item.active');
    if (activeSlide) {
...
$('#imgApplyFilter').click(function () {
    ...
    $("#carouselEnergyTrendHotIndicators .carousel-indicators").empty();
    $("#carouselEnergyTrendHotIndicators .carousel-inner").empty();
        ...
        $.each(allSelectedGraph, function (key, value) {
            ...
            if (frequencyEnable == true && configData) {
                ...
                if (showActual) {
                    ...
                    var actualText = "Daily Actual " + selectedGraphText;
                    var actualContainerID = "HourlyActual" + baseContainerID;
                    var activeClass = slideIndex === 0 ? " active" : "";

                    var divIndicator = '<button type="button" class="' + activeClass + '" data-bs-target="#carouselEnergyTrendHotIndicators" style="background-color:black;" data-bs-slide-to="' + slideIndex + '" aria-label="Slide ' + (slideIndex + 1) + '"></button>';
                    $("#carouselEnergyTrendHotIndicators .carousel-indicators").append(divIndicator);

                    setCorousalItem(activeClass, actualText, actualContainerID);
                    energyHotDashboard.init(actualId, actualContainerID);
                ...
                if (showAverage) {
                    ...
                    var avgText = "Daily Average " + selectedGraphText;
                    var avgContainerID = "HourlyAverage" + baseContainerID;
                    var activeClassAvg = slideIndex === 0 ? " active" : "";

                    var divIndicatorAvg = '<button type="button" class="' + activeClassAvg + '" data-bs-target="#carouselEnergyTrendHotIndicators" style="background-color:black;" data-bs-slide-to="' + slideIndex + '" aria-label="Slide ' + (slideIndex + 1) + '"></button>';
                    $("#carouselEnergyTrendHotIndicators .carousel-indicators").append(divIndicatorAvg);

                    setCorousalItem(activeClassAvg, avgText, avgContainerID);
                    energyHotDashboard.init(avgId, avgContainerID);
                ...
                energyHotDashboard.init(graphId, baseContainerID);
    ...
    if (needsFrequency) {
        $.getJSON('/v2.0/Energy/warm/js/EnergyConsumption_Graph_Config.json', function (data) {
            buildSlides(data);
        });
    } else {
        buildSlides(null);
...
$(document).ready(function () {
    ...
    $('#carouselEnergyTrendHotIndicators').on('slid.bs.carousel', function () {
...
// and calls energyHotDashboard.refreshFilter(graphId) with the exact graphId
...
function refreshFilterForActiveSlide() {
    var carousel = document.querySelector('#carouselEnergyTrendHotIndicators');
    if (!carousel) return;

    // Find active slide index from Bootstrap's active indicator button
    var activeIndicator = document.querySelector('#carouselEnergyTrendHotIndicators .carousel-indicators button.active');
    ...
    if (typeof energyHotDashboard !== 'undefined' && energyHotDashboard.refreshFilter) {
        console.log('🔄 refreshFilterForActiveSlide → slideIndex:', slideIndex, '→ graphId:', graphId);
        energyHotDashboard.refreshFilter(graphId);
    }
}

// ===========================
// CAROUSEL ITEM BUILDER (restored to old working version with img1-header and title)
// ===========================
function setCorousalItem(activeClass, selectedGraphText, graphContainerID) {
    ...
    $("#carouselEnergyTrendHotIndicators .carousel-inner").append(carousalItem);
}

// ===========================
// FILTER ITEM BUILDER (restored to old working version - direct trigger, no setTimeout)
// ===========================
